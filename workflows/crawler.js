...
let dataset = [
  {
    '000': 'https://atlas.cs.brown.edu/data/gutenberg/3/2/'},
...
const doMapReduce = () => {
  distribution.crawler.store.get({batch:batch}, (e, v) => {
    console.log('Values and Error: ', e, v);
    distribution.crawler.mr.exec({keys: v, map: mapCrawlParent,
      reduce: reduceCrawlParent, storeReducedValue: false, batch: batch}, (e, v) => {
      if (v.length != 0) {
...
const doCrawlText = () => {
  ...
  distribution.crawler.store.get({batch:batch}, (e, v) => {
    ...
    distribution.crawler.mr.exec({keys: v, map: mapCrawlText,
      reduce: reduceCrawlText, storeReducedValue: false, batch: batch}, (e, v) => {
      terminate();
    });
  });
};
...
distribution.node.start((server) => {
  localServer = server;
  const crawlerConfig = {gid: 'crawler', batch: batch};
  startNodes(() => {
    groupsTemplate(crawlerConfig).put(crawlerConfig,
        crawlerGroup, (e, v) => {
          ...
          dataset.forEach((o) => {
            ...
            let value = o[key];
            distribution.crawler.store.put(value, {key:key, batch:batch}, (e, v) => {
              if (!e) {
                cntr++;
